// Head skin texture baking.
//
// Mirrors the fragment-shader color assignment in
// `hs2_head_head_material.wgsl`: sample the MainTex and apply the simple HSV
...
@group(0) @binding(60) var head_texture: texture_2d<f32>;
@group(0) @binding(61) var head_sampler: sampler;
...
fn bake(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
  ...
  let main_color = textureSampleLevel(head_texture, head_sampler, uv, 0.0);
