...
use bevy::{
    ...
    render::{
        ...
        render_resource::{
            Buffer, BufferDescriptor, BufferUsages, CommandEncoderDescriptor, Extent3d, Maintain, MapMode, TexelCopyBufferInfo, TexelCopyBufferLayout, TextureDimension, TextureFormat, TextureUsages
        },
        ...
        if n < 1 {
            ...
            if !image_data.is_empty() {
                for image in images_to_save.iter() {
                    ...
                    info!("saveing image to: {images_dir:?}");
...
fn setup(
    ...
    let render_target = setup_render_target(
        ...
        // To visualize stages of scene rendering chnage this param to 0
        // and change AppConfig::single_image to false in main
        // Stages are:
        // 1. Trasparent image
        // 2. Fwe black box images
        // 3. Fully rendered scene images
        // Exact number dependds on device speed, device load and scene size
        40,
        "main_scene".into(),
    );
